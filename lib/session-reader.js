...
export class SessionUsageReader {
  ...
  async readLatest(cancellable = null) {
    ...
    for (const item of recent) {
      ...
      // Codex normally writes both the 5-hour and weekly windows into the
      // same token_count event. Stop as soon as both are known instead of
      // reading every recent transcript in full. This keeps cold-start memory
      // and latency bounded even when session JSONL files grow very large.
